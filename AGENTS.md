...
- Build: `python -m PyInstaller jvav_dk27.spec --clean --noconfirm`, then copy `dist\jvav_dk27.exe` to `downloads\`. `build/` and `dist/` are gitignored.
...
- The current test suite is a single file with 5 tests (`tests/test_repl_function.py`).
- `jvav_dk27.spec` bundles `assets/logo.ico` and lists `hiddenimports` for the module system; update both if imports are added.
